...
def _build_message(side: str, trow: dict, symbol: str = None) -> str:
    ...
    umid_val = _as_float(trow.get("UpperMid"))
    lmid_val = _as_float(trow.get("LowerMid"))
    vup_val = _as_float(trow.get("ValueUpper"))
    vlo_val = _as_float(trow.get("ValueLower"))
    uq_val = _as_float(trow.get("UpperQ"))
    lq_val = _as_float(trow.get("LowerQ"))

    # MarkdownV2 seguro
    title = _escape_md(f"{sym} {interval} | Touch {side}Q")
    lines = [f"*{title}*"]

    info_line = _escape_md(f"Fecha: {date} | Simbolo: {sym} | TF: {interval}")
    lines.append(info_line)

    if side.lower() == "upper":
        touch_price = _fmt_price(uq_val if uq_val is not None else trow.get("ValueUpper"))
        upper_mid = _fmt_price(umid_val)
        value_upper = _fmt_price(vup_val)
        detail_line = _escape_md(f"TouchUpperQ: {touch_price} | UpperMid: {upper_mid} | ValueUpper: {value_upper}")
        lines.append(detail_line)
        if umid_val is not None and lmid_val is not None and lmid_val != 0:
            diff_pct = None
            try:
                diff_pct = ((umid_val - lmid_val) / abs(lmid_val)) * 100.0
            except Exception:
                diff_pct = None
            if diff_pct is not None:
                diff_line = _escape_md(f"Rango UpperMid↔LowerMid: {_fmt_pct(diff_pct)}")
                lines.append(diff_line)
    else:
        touch_price = _fmt_price(lq_val if lq_val is not None else trow.get("ValueLower"))
        lower_mid = _fmt_price(lmid_val)
        value_lower = _fmt_price(vlo_val)
        detail_line = _escape_md(f"TouchLowerQ: {touch_price} | LowerMid: {lower_mid} | ValueLower: {value_lower}")
        lines.append(detail_line)
        if umid_val is not None and umid_val != 0 and lmid_val is not None:
            diff_pct = None
            try:
                diff_pct = ((umid_val - lmid_val) / abs(umid_val)) * 100.0
            except Exception:
                diff_pct = None
            if diff_pct is not None:
                diff_line = _escape_md(f"Rango LowerMid↔UpperMid: {_fmt_pct(diff_pct)}")
                lines.append(diff_line)
...
def send_touch_alert(side: str, trow: dict, symbol: str = None):
    ...
    Parámetros:
      - side: "Upper" o "Lower"
      - trow: dict con las claves Date, Open, High, Low, Close, UpperMid, ValueUpper, LowerMid, ValueLower, UpperQ, LowerQ
