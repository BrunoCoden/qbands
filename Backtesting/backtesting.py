...
BACKTEST_WARN_TOO_MUCH = int(os.getenv("BACKTEST_WARN_TOO_MUCH", "100000"))
...
def _touch_markers(series: pd.Series, low: pd.Series, high: pd.Series) -> pd.Series:
    touched = (low <= series) & (high >= series)
    markers = pd.Series(np.nan, index=series.index)
    if touched.any():
        markers.loc[touched] = series.loc[touched]
    return markers
...
# === Plotting ================================================================
def plot_trades(price_df: pd.DataFrame, trades_df: pd.DataFrame, output_path: Optional[Path]) -> Optional[Path]:
    ...
    add_if(_linebreak_like(staged.get("ValueUpper")), color="#1dac70", width=1)
    add_if(_linebreak_like(staged.get("Value")), color="gray", width=1)
    add_if(_linebreak_like(staged.get("ValueLower")), color="#df3a79", width=1)

    add_if(_linebreak_like(staged.get("UpperMid")), color="gray", width=1, alpha=0.5)
    add_if(_linebreak_like(staged.get("LowerMid")), color="gray", width=1, alpha=0.5)

    add_if(_linebreak_like(staged.get("UpperQ")), color="yellow", width=1, linestyle=":")
    add_if(_linebreak_like(staged.get("LowerQ")), color="yellow", width=1, linestyle=":")

    add_if(_linebreak_like(staged.get("TrendSMA")), color="#00bcd4", width=1.2, alpha=0.9)

    if _has_data(staged.get("UpperQ")):
        suq = _touch_markers(staged["UpperQ"], staged["Low"], staged["High"])
        add_if(suq, type="scatter", marker="o", markersize=40, color="white")
    if _has_data(staged.get("LowerQ")):
        slq = _touch_markers(staged["LowerQ"], staged["Low"], staged["High"])
    ...
    for trade in trades_df.itertuples():
        ...
        if entry_time not in staged.index:
            continue
        if str(trade.exit_reason) == "target_hit":
            entries_win.loc[entry_time] = trade.entry_price
            if exit_time in staged.index:
    ...
    add_if(exits_win, type="scatter", marker="v", color="#3fff9d", markersize=70)
    add_if(entries_loss, type="scatter", marker="^", color="#ff4d4d", markersize=80)
    add_if(exits_loss, type="scatter", marker="v", color="#ff4d4d", markersize=70)

    savefig = None
    if output_path:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        savefig = dict(fname=str(output_path), dpi=150)
    ...
    plot_kwargs = dict(
        ...
        warn_too_much_data=BACKTEST_WARN_TOO_MUCH,
    )
    if savefig:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Ruta para guardar el gráfico generado (si se omite, solo se muestra en pantalla).",
    )
    ...
    plot_result = plot_trades(history, trades_df, plot_path)
    if plot_result:
        print(f"Gráfico guardado en {plot_result}")
